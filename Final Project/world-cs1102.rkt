...
|#
;; Mon Sep 5  15:54:23 EDT 2016: rename big-bang to create-canvas
...
(provide      ;; forall(World):
 create-canvas     ;; Number Number -> true
 )
...
(define (create-canvas w h)
  (check-size/0 'create-canvas w "first")
  (check-size/0 'create-canvas h "second")
  (let ([w (coerce w)]
	[h (coerce h)])
    (when *running?*  (error 'create-canvas "the world is still running, cannot start another world"))
    (set! *running?* #t)
    (callback-stop!)
    ;; (when (vw-init?) (error 'create-canvas "create-canvas already called once"))
    (install-world 0 null) ;; call first to establish a visible world
    (set-and-show-frame w h #f) ;; now show it
    #t))
...
(define (check-world tag)
  (when (eq? unique-world the-world) 
    (tp-error tag "create-canvas has not been called before calling ~a" tag)))
